...
#
# The ledger additionally separates schema ownership from runtime access (ADR-007):
#   ledger_app      owns the schema and runs migrations (DDL).
#   ledger_runtime  used by ledger-api at runtime: SELECT/INSERT/UPDATE only. No DELETE,
#                   TRUNCATE or DDL, and as a non-owner it cannot disable the ledger's triggers.
set -euo pipefail
